---
name: Spot VMs | Compute Engine - Google Cloud Documentation
...
---
Spot VMs | Compute Engine | Google Cloud Documentation
...
Was this helpful?
Send feedback
Spot VMs Stay organized with collections Save and categorize content based on your preferences. Dismiss Got it
On this page
What are Spot VMs?
Spot VMs limitations
Preemption of Spot VMs
...
Spot VMs are VMs that use the spot provisioning model. This provisioning model lets you obtain resources at much lower prices compared to standard VMs. However, Compute Engine can preempt Spot VMs at any time to reclaim resources. Use Spot VMs to reduce costs for fault-tolerant workloads.
...
What are Spot VMs?
Spot VMs are available at much lower prices— up to 91% discounts for many machine types, GPUs, TPUs, and Local SSDs—compared to the on-demand price for standard VMs.
However, Compute Engine might preempt Spot VMs at any time, such as when it needs the resources for other tasks. At this uncertain preemption time, Compute Engine either stops (default) or deletes your Spot VMs depending on your specified termination action for each Spot VM. Spot VMs are excess Compute Engine capacity, so their availability varies based on Compute Engine usage. Spot VMs don't have a minimum or maximum runtime unless you specifically limit the runtime.
...
Bare metal instances
The Google Cloud Free Tier credits for Compute Engine don't apply to Spot VMs.
...
Additional limitations apply to TPU Spot VMs.
Preemption of Spot VMs
This section describes how Compute Engine preempts Spot VMs and which VMs are selected for preemption. Additionally, instructions to detect, handle, and test preemption of Spot VMs are explained later in Manage preemption of Spot VMs.
Preemption process
Compute Engine performs the following steps to preempt Spot VMs:
Compute Engine sends a preemption notice to the VM by updating the preempted value in your VM's default metadata to TRUE , which your workload can monitor to detect preemption. The preemption notice duration is the duration between this metadata signal and when Compute Engine sends a ACPI G2 Soft Off signal to begin the shutdown period. The preemption notice duration affects how your workload can handle preemption and can be set to one of the following values while you are creating a Spot VM:
120 seconds ( Preview): We recommend setting the preemption notice duration to 120 seconds for any workloads that need a dedicated duration or longer than 30 seconds to handle preemption.
0 seconds (default): If the preemption notice duration for a Spot VM isn't specified or is set to 0, then there is no dedicated delay between detecting preemption in metadata and the ACPI G2 Soft Off signal. Your workload can handle the preemption notice during the following shutdown period by using a shutdown script.
When the ACPI G2 Soft Off signal is sent, it triggers any shutdown script that you have configured for the Spot VM and represents the beginning of the shutdown period. The shutdown period for Spot VMs is best effort and up to 30 seconds, which is shorter than the shutdown period for other instances.
If the Spot VM hasn't stopped after the shutdown period, Compute Engine sends an ACPI G3 Mechanical Off signal to the operating system.
...
You can't view why any given preemption event occurred. Similar to resource availability errors, preemption rates can vary in different zones, at different times, or with different resources. For specific suggestions on reducing your preemption rate, see the best practices.
Pricing
Spot prices, the prices for Spot VMs, provide significant discounts for VMs. Spot prices give you discounts of up to 91% off of the on-demand price for many machine types, GPUs, TPUs, and Local SSDs. Important: Spot prices can change up to once every day. Spot prices don't appear in most pricing tables for Compute Engine. For the latest prices, see the Spot VMs pricing page.
...
Local SSDs on Spot VMs
You can start Spot VMs with local SSD disks and Compute Engine charges you spot prices for the local SSD disk usage. Local SSD disks attached to Spot VMs work like normal local SSD disks, retain the same data persistence characteristics, and remain attached for the life of the VM.
Compute Engine doesn't charge you for local SSD disks if their VMs are preempted in the first minute after they start running.
For more information about local SSD disks, see About Local SSD disks.
GPUs on Spot VMs
